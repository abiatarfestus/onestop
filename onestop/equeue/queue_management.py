...
class Queue:
    ...
    def form_queue(self, pk, join_message=None):
        ...
        self.current_queue = QueuedCustomer.objects.filter(
            service_enrolment_id=pk
        ).order_by(
            "join_time"
        )  # A queryset of queuedCustomer instances filtered by the service_id
        ...
        if self.request.method == "POST":
            if self.request.user in self.queued_users:
                ...
                QueuedCustomer.objects.create(
                    customer=self.request.user, service_enrolment=self.service_enrolment
                )
...
class Customer:
    ...
    def exit_queue(self, pk):
        ...
        self.queued_instance = QueuedCustomer.objects.filter(
            service_enrolment_id=pk, customer_id=self.request.user.id
        ).get()
        CancelledCustomer.objects.create(
            service_enrolment_id=self.queued_instance.service_enrolment_id,
            customer_id=self.queued_instance.customer.id,
...
class Servant:
    ...
    def my_queues(self, pk):
        ...
        for (
            ...
            self.servant_enrolments
        ):  # Loops through the servant_enrolments, extract the service_enrolment ids and add them to the list
            self.service_enrolment_ids.append(entry.service_enrolment_id)
        self.eligible_queues = QueuedCustomer.objects.filter(
            service_enrolment_id__in=self.service_enrolment_ids
        )
    ...
    def serve_customers(self, pk, current_customer=0):
        ...
        self.context = {
            ...
            "last_served_id": current_customer,
        }
        if current_customer > 0:
            served_customer = ServedCustomer.objects.get(id=current_customer)
            self.context["current_customer"] = User.objects.get(
                id=served_customer.customer_id
            )
        self.service_enrolment = ServiceEnrolment.objects.get(id=pk)
        self.current_queue = QueuedCustomer.objects.filter(
            service_enrolment_id=pk
    ...
    def next_customer(self, pk, last_served_id):
        ...
        ServedCustomer.objects.create(
            customer_id=self.front_customer_id,
            service_enrolment_id=self.front_customer.service_enrolment_id,
        ...
        current_customer = ServedCustomer.objects.filter(
            ...
            service_enrolment_id=self.front_customer.service_enrolment_id,
            served_by=self.request.user,
        ).order_by("-date_time_served")
        served_id = current_customer[0].id
        QueuedCustomer.objects.filter(id=pk).delete()
        return redirect(
            "equeue:serve-customers",
            pk=self.front_customer.service_enrolment_id,
            current_customer=served_id,
        )
